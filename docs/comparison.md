...
| --- | :---: | :---: | :---: | :---: |
| **Orphaned user detection** | âœ… | âœ… | âœ… | âš  custom code required |
| **Risk scoring (HIGH / MEDIUM / LOW)** | âœ… | âœ… | â€” | â€” |
| **HTML report** | â€” | âœ… | âœ… web UI | â€” |
| **WhatIf / dry-run** | âœ… | âœ… | â€” | âš  manual implementation |
| **Snapshot & restore** | â€” | âœ… | âœ… permission history | â€” |
| **Scheduled / unattended scan** | â€” | âœ… | âœ… continuous | âš  manual Task Scheduler setup |
| **Pricing** | $0 | $79 / tenant / year | per-user SaaS (varies) | free (time investment) |
| **PowerShell pipeline support** | âœ… | âœ… | â€” | âœ… |
| **Air-gapped / offline operation** | âœ… | âœ… | â€” | âœ… |

âš  = partial support; â€” = not available

---

## Positioning

**SPClean** is a focused, single-purpose module: detect orphaned users, score their risk, and remove them safely with rollback support. It lives inside your existing PowerShell workflow, integrates with the pipeline, and runs entirely inside your tenant â€” no SaaS backend, no data leaving your environment. Licensing is flat per-tenant ($79/year for Pro), which makes the cost predictable regardless of how many users your tenant has.
...
**Manual PowerShell scripting** has no licensing cost and gives full control, but building a reliable implementation â€” cross-referencing UILs with Entra ID via Graph batch, classifying account states, handling soft-deleted accounts, writing structured output, and wiring up WhatIf â€” takes substantial effort to develop and maintain. SPClean is effectively that script, already built and tested.
