...
- **License key format** — keys are now standard UUIDs (`XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX`) generated by Gumroad. Existing `SPCLEAN-PRO-…` / `SPCLEAN-CONSULTANT-…` keys from v1.1.x are no longer valid; purchase a new key at `https://hungpham2802.gumroad.com`.
