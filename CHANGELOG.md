...
### Fixed
- **Export-SPCStorageReport / New-SPCStorageDashboardHtmlInternal**: Fixed a JavaScript syntax error caused by PowerShell here-string escaping ES6 template literal backticks, ensuring the Top Storage Waste Sites table and interactive ROI elements populate accurately across all browsers.
...
### Fixed
- **Get-SPCGraphSiteUsageInternal**: Auto-resolve site URLs via PnP SiteId when M365 privacy concealment is enabled in Microsoft 365 Admin Center reports.
- **Connect-SPCSiteInternal**: Enhanced interactive token resolution to acquire dedicated SharePoint resource tokens for individual site connections and gracefully filter out redirect sites.
- **Get-SPCStorageWaste**: Properly hydrate CSOM Site.Usage properties via Get-PnPProperty to retrieve exact storage metrics in PnP PowerShell 3.2+.
- **Get-SPCPreservationHoldWaste**: Ensured resilient scanning when auditing storage waste on preservation hold sites.
...
### Fixed
- **Get-SPCMismatchUser**: Fixed null AadObjectId handling by defaulting safely to Healthy to avoid accidentally removing active users. Translated Vietnamese comments to English.
...
### Changed
- **License verification rewritten** � replaced offline HMAC-SHA256 engine with Gumroad License API (`POST https://api.gumroad.com/v2/licenses/verify`). Internet access is required on first activation and every 7 days for re-verification.
- **License key format** � keys are now standard UUIDs (`XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX`) generated by Gumroad. Existing `SPCLEAN-PRO-�` / `SPCLEAN-CONSULTANT-�` keys from v1.1.x are no longer valid; purchase a new key at `https://ngochung47.gumroad.com/`.
...
### Removed
- `tools/New-SPCLicenseKey.ps1` and `tools/Inject-Secret.ps1` � no longer needed; key generation and management is handled by Gumroad.
