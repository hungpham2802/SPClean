function Export-SPCReport {
    ...
    begin {
        ...
        if ($BrandingName) {
            Assert-SPCConsultantLicense -Feature 'BrandingName'
        }
...
"@
                ...
                $appName = if ($BrandingName) { $BrandingName } else { "SPClean" }
                $html  = @"
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>$appName Report - $tenantName</title>
<style>
...
<body>
<h1>$appName - Orphaned User Report</h1>
$summHtml
<div class="wrap">
<table id="t">
<thead><tr>
...
<footer>Generated by $appName $(if (-not $BrandingName) { "v$moduleVer " })&bull; Tenant: $tenantName &bull; $tsStr UTC</footer>
<script>
(function(){
