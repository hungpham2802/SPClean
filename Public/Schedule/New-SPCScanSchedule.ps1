    ...
    process {
        ...
        if ([string]::IsNullOrWhiteSpace($CertificateThumbprint)) {
            if ([string]::IsNullOrWhiteSpace($CertificatePath) -or $null -eq $CertificatePassword) {
                throw "New-SPCScanSchedule: You must provide either -CertificateThumbprint, or BOTH -CertificatePath and -CertificatePassword."
            }
        ...
        $encryptedPwd = if ($null -ne $CertificatePassword) { $CertificatePassword | ConvertFrom-SecureString } else { $null }
        ...
        $authParams = if (-not [string]::IsNullOrWhiteSpace($CertificateThumbprint)) {
        ...
        $pwdDecoding = if ($null -ne $encryptedPwd) { "`$CertificatePassword = '$encryptedPwd' | ConvertTo-SecureString" } else { "" }
